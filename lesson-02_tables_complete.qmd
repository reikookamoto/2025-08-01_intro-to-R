...
-   `table1`, `gtsummary` - These are both packages used to create tables with descriptive statistics (another example is `tableone` but won't be covered today)
...
CCHS <- readr::read_csv(file = "cchs2013_2014_p_subset.csv", col_names = T, progress = T)

#Can read in file using read.csv() command or readr::read_csv() command (example below)
#CCHS=read.csv(file="cchs2013_2014_p_subset.csv", header=TRUE)
```
...
-   💡 Hint: Heart disease is variable `CCC_121`. Non-missing values are denoted as 1 and 2.

```{r study_cohort_creation}

CCHS_CUT <- CCHS |> filter(DHHGAGE>=10 & CCC_121 %in% c(1,2))
...
```{r recode_diabetes}
CCHS_CUT$Diabetes <- ifelse(CCHS_CUT$CCC_101 == 1, 1, 
                           ifelse(CCHS_CUT$CCC_101 == 2, 0, 
                                  NA))
...
🚶‍♀️ Recode Heart disease variable (denoted as `CCC_121` in our dataset) to 1 (has heart disease) and 0 (no heart disease)

```{r recode_heart}
CCHS_CUT$Heart <- recode(CCHS_CUT$CCC_121, `1` = 1, `2` = 0, .default = NA_real_) 

```

🚶‍♀️ Create a new Daily Fruit Consumption variable (from original variable `FVCDFRU`) where we assign NA to missing values (i.e., 999.9), otherwise it'll throw off calculations of mean, etc.

```{r _recode_fruit}
CCHS_CUT$Fruit <- ifelse(CCHS_CUT$FVCDFRU == 999.9, NA, 
                           CCHS_CUT$FVCDFRU)
...
-   💡 Hint: a value of 1 denotes a male, a value of 2 denotes a female.

```{r Factoring_Sex}


CCHS_CUT$DHH_SEX = factor(CCHS_CUT$DHH_SEX, levels=c(1,2), 
...
var_label(CCHS_CUT$DHH_SEX) = "Sex"
```
...
🚶‍♀️ Make a table below showing the baseline characteristics of age (`DHHGAGE`), sex (`DHH_SEX`), diabetes, heart disease, and fruit consumption for the cohort

```{r template}

table1(~DHHGAGE + DHH_SEX + Diabetes + Heart + Fruit, data=CCHS_CUT)
...
table1(~DHHGAGE + Diabetes + Heart + Fruit | DHH_SEX, data=CCHS_CUT)
...
table1( ~ DHHGAGE + DHH_SEX + Diabetes + Fruit | Heart, data = CCHS_CUT)

```

💡 By default, `table1` will output an "Overall" column. If we don't want that overall column, we can add the `overall=FALSE` option.

🚶‍♀️ Make a descriptive table with age, diabetes, and fruit, stratified by heart disease, but no overall column and now specifying median & IQR for continuous variables (hint: `render.continuous=c("Median [IQR]"="Median [Q1, Q3]")`) and also having these values be rounded to 2 significant digits (via `digits=2`)
...
🚶‍♀️ Create a descriptive table of age (`DHHGAGE`), sex (`DHH_SEX`), diabetes, and fruit consumption

```{r gtsummary_table}

CCHS_CUT |>
  tbl_summary(include = c(DHHGAGE, DHH_SEX, Diabetes, Fruit))
...
🚶‍♀️ Create a descriptive table of age (`DHHGAGE`), sex (`DHH_SEX`), diabetes, and fruit consumption, stratified by heart disease, with p-values (`add_p()`) and overall column (`add_overall()`) displayed.
...
model1 <- glm(Heart ~ Diabetes + DHHGAGE + DHH_SEX, CCHS_CUT, family = binomial)
...
table_model <- tbl_regression(model1, exponentiate = TRUE)
